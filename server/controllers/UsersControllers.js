...
const colors = ['#7EDE28', '#66CD00', '#CDD704', '#FF3D0D', '#BE2625', '#34C2A1', '#0FDDAF', '#0198E1', '#2619EF', '#AD0AFD', '#F57C00', '#E91E63', '#FFC107', '#005555'];
...
class UsersController {
    ...
    checkInfor(req, res) {
        ...
        let u;
        try {
            u = Helper.decryptBase64(provideAttribute.u);

            res.send(Status.getStatus('success', 'Successful', JSON.parse(u)));
    ...
    checkLogin(req, res) {
        ...
        return Users.findOne({
            ...
            attributes: ['id', 'email', 'type', 'thumbnail', 'name']
        }).then(rs => {
            if (rs) {
                let { id, email, thumbnail, name } = rs;
                var token = Helper.generateTokenById(id, 'client', email, thumbnail, name);
                var u = Helper.encryptBase64(JSON.stringify({ id, type: 'client', email, thumbnail, name }));
    ...
    findAll(req, res) {
        return Users.findAll({
                attributes: ['id', 'email', 'type', 'name', 'thumbnail', 'createdAt', 'updatedAt']
            })
            .then(rs => {
                res.send(Status.getStatus("success", "Successful", rs));
            })
    ...
    getSuggest(req, res) {
        let provideAttributes = Helper.checkPostProvidedAttribute(req, res, ['ignore']);
    ...
    search(req, res) {
        let provideAttributes = Helper.checkPostProvidedAttribute(req, res, ['q', 'ignore']);
